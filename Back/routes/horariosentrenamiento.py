...
def get_horarios_usuario(id_usuario):
    ...
    try:
        ...
        cursor.execute(
            "SELECT * FROM horariosentrenamiento WHERE ID_Usuario = %s", 
            (id_usuario,)
        )
        horarios = cursor.fetchall()
        return jsonify(horarios)
...
def put_horario(id_horario):
    ...
    try:
        ...
        cursor.execute("SELECT 1 FROM horariosentrenamiento WHERE ID_HorarioEntrenamiento = %s", (id_horario,))
        if not cursor.fetchone():
            return jsonify({"error": "Horario no encontrado"}), 404
            
        if "ID_Plan" in updates:
            cursor.execute("SELECT 1 FROM planes WHERE ID_Plan = %s", (updates["ID_Plan"],))
            if not cursor.fetchone():
                return jsonify({"error": "Plan no encontrado"}), 404
...
def delete_horario(id_horario):
    ...
    try:
        ...
        cursor.execute("DELETE FROM horariosentrenamiento WHERE ID_HorarioEntrenamiento = %s", (id_horario,))
        conn.commit()
